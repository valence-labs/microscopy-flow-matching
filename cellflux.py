...
"""The OpenAI ADM UNet (directly copied from the CellFlux code for reproducibility)."""
...
class ADMWrapper(nn.Module):
    def __init__(self, input_size, y_dim, e_dim, c_dim, in_channels=6):
        super().__init__()
        del input_size
        self.adm = UNetModel(
            **{  # configs copied from https://github.com/yuhui-zh15/CellFlux/blob/041c4c071056896316338872145cef164b07a5fc/models/model_configs.py
        ...
        self.adaptor = BasicAdaptor(
            hidden_size=200, y_dim=y_dim, e_dim=e_dim, c_dim=c_dim
        )

    def forward(self, *, x, t, y=None, e=None, c=None):
        t_null = torch.zeros_like(t)  # don't use the adaptor for t
        cond = self.adaptor(t_null, y, e, c)
        return self.adm(x, t, extra={"concat_conditioning": cond})